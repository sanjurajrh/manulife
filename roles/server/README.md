...
The role uses the `redhat.rhel_system_roles.firewall` role to manage the firewall, hence, only firewall implementations supported by the `redhat.rhel_system_roles.firewall` role work.
...
Use this variable to generate certificate and private key for TLS encryption using the `redhat.rhel_system_roles.certificate`.
...
Configuring for high availability is not supported on RHEL 7 because the `redhat.rhel_system_roles.ha_cluster` role does not support RHEL 7.
...
If you set [mssql_ha_cluster_run_role](#mssql_ha_cluster_run_role) to `true`, you can provide variables required by the `redhat.rhel_system_roles.ha_cluster` role.
If you do not provide names or addresses, the `redhat.rhel_system_roles.ha_cluster` uses play's targets, and the high availability setup requires pacemaker to be configured with short names.
Therefore, if you define hosts in inventory not by short names, or the default hosts' IP address differs from the IP address that pacemaker must use, you must set the corresponding `redhat.rhel_system_roles.ha_cluster` role variables.

For an example inventory, see [Example Inventory for HA Configuration](#Example-Inventory-for-HA-Configuration).

See the `redhat.rhel_system_roles.ha_cluster` role's documentation for more information.
...
1. Include the `redhat.rhel_system_roles.firewall` role to configure firewall:
...
2. Configure SQL Server for high availability:
     ...
     4. Configure the user provided with the [mssql_ha_login](#mssql_ha_login) variable for
        Pacemaker.
3. Optional: Include the `redhat.rhel_system_roles.ha_cluster` role to configure Pacemaker.
You must set [mssql_ha_cluster_run_role](#mssql_ha_cluster_run_role) to `true` and provide all variables required by the `redhat.rhel_system_roles.ha_cluster` role for a proper Pacemaker cluster configuration based on example playbooks in [Setting Up SQL Server and Configuring for High Availability](#Setting_Up_SQL_Server_and_Configuring_for_High_Availability).
...
Whether to run the `redhat.rhel_system_roles.ha_cluster` role from this role.

Note that the `redhat.rhel_system_roles.ha_cluster` role has the following limitation:
...
To work around this limitation, the `microsoft.sql.server` role does not set any variables for the `redhat.rhel_system_roles.ha_cluster` role to ensure that any existing Pacemaker configuration is not re-written.

If you want the `microsoft.sql.server` to run the `redhat.rhel_system_roles.ha_cluster` role, set `mssql_ha_cluster_run_role: true` and provide variables for the `redhat.rhel_system_roles.ha_cluster` role with the `microsoft.sql.server` role invocation based on example playbooks in [Setting Up SQL Server and Configuring for High Availability](#Setting_Up_SQL_Server_and_Configuring_for_High_Availability).

If you do not want the `microsoft.sql.server` to run the `redhat.rhel_system_roles.ha_cluster` role and instead want to run the `redhat.rhel_system_roles.ha_cluster` role independently of the `microsoft.sql.server` role, set `mssql_ha_cluster_run_role: false`.
...
If you want to configure Pacemaker from this role, you can set [mssql_ha_cluster_run_role](#mssql_ha_cluster_run_role) to `true` and provide variables required by the `redhat.rhel_system_roles.ha_cluster` role to configure Pacemaker for your environment properly.

This example configures required Pacemaker properties and resources and enables SBD watchdog.

The `redhat.rhel_system_roles.ha_cluster` role expects watchdog devices to be configured on `/dev/watchdog` by default, you can set a different device per host in inventory.
For more information, see the `redhat.rhel_system_roles.ha_cluster` role documentation.
...
If you want to configure Pacemaker from this role, you can set [mssql_ha_cluster_run_role](#mssql_ha_cluster_run_role) to `true` and provide variables required by the `redhat.rhel_system_roles.ha_cluster` role to configure Pacemaker for your environment properly.
See the `redhat.rhel_system_roles.ha_cluster` role documentation for more information.
...
This example playbooks sets the `firewall` variables for the `redhat.rhel_system_roles.firewall` role and then runs this role to open the probe port configured in Azure.
...
This role uses the `redhat.rhel_system_roles.ad_integration` role to join SQL Server with AD server.
...
* Optional, You can configure DNS using ad_integration role by providing the following variables:
  ...
  ```
* Optional: You can provide further variables for the `redhat.rhel_system_roles.ad_integration` role if you need.
